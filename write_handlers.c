...
{ /* char is stored at left and paddind at buffer's right */
	...
	if (width > 1)
	{
		...
		if (flags & F_MINUS)
			return (_write(1, &buffer[0], 1) +
					_write(1, &buffer[BUFF_SIZE - i - 1], width - 1));
		else
			return (_write(1, &buffer[BUFF_SIZE - i - 1], width - 1) +
					_write(1, &buffer[0], 1));
	}
...
int write_num(int ind, char bff[], int flgs, int w, int l, char pd, char ex_c)
{
	...
	if (w > l)
	{
		...
		else if (!(flgs & F_MINUS) && pd == '0')/* extra char to left of padding */
		{
			if (ex_c)
				bff[--padd_start] = ex_c;
			return (_write(1, &bff[padd_start], i - padd_start) +
				_write(1, &bff[ind], l - (1 - padd_start)));
		}
	}
	if (ex_c)
		bff[--ind] = ex_c;
...
char buffer[], int flags, int width)
{
	...
	if (width > length)
	{
		...
		{
			return (_write(1, &buffer[ind], length) +
					_write(1, &buffer[0], i));
		}
		else /* Asign extra char to left of padding [padd>buffer]*/
		{
			return (_write(1, &buffer[0], i) +
					_write(1, &buffer[ind], length));
		}
	}
